#!/usr/bin/env python3
"""Non-interactive QSTE build and optional PyPI upload helper.

Build only (default) -- version bump, install, tests, build, twine check::

    python release.py --keep-version
    python release.py --release patch          # 0.1.0 -> 0.1.1

Upload after every local gate passes::

    python release.py --release patch --upload
    python release.py --keep-version --upload --testpypi   # rehearse on TestPyPI

Nothing is uploaded unless ``--upload`` is passed, and upload is refused if the
tests did not run or the LICENSE is missing.
"""
...
INIT_FILE = ROOT / "src" / "qste" / "__init__.py"
DIST_DIR = ROOT / "dist"
...
def main() -> int:
    ...
    if args.release:
        target = (
            bump_version(current, args.release)
    ...
    if not args.upload:
        print("QSTE RELEASE BUILD: PASS (nothing uploaded)")
        return 0
    if args.skip_tests:
        raise SystemExit("Upload refused: tests were skipped; drop --skip-tests to upload.")
    ...
    run(command)
    print(f"QSTE {read_version()} UPLOAD: PASS")
    return 0
